...
hdp_extract_components <- function(x, cos.merge = 0.90, min.sample = 1) {
  ...
  if (!is.numeric(cos.merge) || cos.merge >= 1L || cos.merge <= 0) {
    stop("cos.merge must be between 0 and 1")
  }
  if (min.sample %% 1L != 0L || min.sample < 1L) {
  ...
  if (is_multi) {
    ...
    if (is_prior) {
      pseudodata <- sapply(
        dp(final_hdpState(chlist[[1]]))[pseudo],
    ...
    rapch_clust <- flexclust::kcca(rapch_unlist,
      k = rapch_ic,
      group = rapch_gf,
    ...
    remove(
      ccc_0, cdc_0, ccc_raw_avg_per_ch, rapch_unlist, rapch_gf, rapch_ic,
  ...
  if (mclust == 1) {
    ...
    ccc_clust <- flexclust::kcca(ccc_unlist,
      k = initial_clust,
      group = groupfactor,
      family = flexclust::kccaFamily("kmedians",
        groupFun = "differentClusters"
  ...
  # to component '0'
  use_clust <- c()
  for (ii in seq_len(ncol(ccc_3[[1]]))) {
  ...
  for (ii in seq_len(ncol(cdc_4[[1]]))) {
  ...
  colorder <- c(1L, setdiff(order(avg_ndi, decreasing = TRUE), 1L))
  ...
  if (is_prior) {
    ...
    avgdistn <- sapply(2:nco, function(i) {
      ...
      ans <- rowMeans(distns, na.rm = TRUE)
      return(ans)
    })
  ...
  ccc_6 <- lapply(ccc_5, function(x) {
    ...
    if (is_prior) {
      ...
      if (length(update) > 0) {
        colnames(x)[update] <- paste0("N", seq_along(update))
      }
    } else {
  ...
  x@comp_categ_distn <- list(
    mean = ccc_mean,
    cred.int = ccc_credint
  )
  x@comp_dp_distn <- list(
    mean = cdc_mean,
